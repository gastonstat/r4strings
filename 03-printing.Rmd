...
In this chapter you will learn about the different functions and options 
to print strings in a variety of formats. A common task when working with
character strings involves printing and displaying them on the screen or on
a file. R allows you to achieve these task using a handful of printing
...
objects' classes. Other functions are useful for printing outputs either in the 
R console or in a given file. In this section we will describe the following 
...
we can set the argument `quote = FALSE` 
...
The output produced by `print()` can be customized with various optional
arguments. However, the way in which `print()` displays objects is
rather limited. To get more printing variety there is a number of more flexible
...
The object is the character string `"Good morning!"`. In fact, this is
a vector of one element. So the behavior of `print()` is the default
behavior when printing any type of character vector.

What about trying to print two vectors?

```{r good_morning_vectors, error = TRUE}
good <- 'good'
morning <- 'morning'
print(good, morning)
```

R throws an error. `print()` is taking the first argument `good` as the object 
to be printed. And then it is taking `morning` as A second argument. Since the 
function that is call is actually the default method `printf.default()`, this 
function attempts to treat the second parameter as the `digits` parameter 
(which is what the error message is showing).
...
for the C library function of the same name. Simply put, `sprintf()` allows 
you to create strings as output using formatted data.

Outputs can be obtained in different formats according to the following table:
...
with the `%0xd` option to indicate the an integer should be padded with `x` 
