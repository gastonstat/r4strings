...
This is a minor but important detail and we will come back to this behavior of r
egular expressions.
...
As you just learned, the most basic type of regular expressions are the
literal characters which are characters that match themselves. However,
not all characters match themselves. Any character that is not a literal
character is a metacharacter. This type of characters have a special meaning
and they allow you to transform literal characters in very powerful ways.

Simply put, everything else that you need to know about regular expressions 
besides literal characters is how do these metacharacters all work. That is
the simple part. The complication lies in the fact that most metacharacters 
have more than one meaning. And learning those meanings takes time.
The meaning of the metacharacters greatly depend on the context in which you
use them. 
...
For example, consider the vector `c("not", "note", "knot", "nut")` and the 
pattern `"n.t"`
...
example: a regular expression formed by `"5.00"`. If you think that this 
...
However, R is a bit different. Ins tead of using a backslash you have to use
