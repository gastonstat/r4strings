...
characters is _character_. Formally, the __mode__ of object that holds 
...
- text elements displayed in plots
...
`read.delim()`. Assuming that the file is in your working directory:
...
creating objects of type `"character"` and testing whether an 
...
If we give `paste()` objects of different length, then it will apply a 
recycling rule. For example, if we paste a single character `"X"` with the
