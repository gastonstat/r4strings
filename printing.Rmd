...
Another very useful function is `cat()` which allows us to concatenate objects 
...
The function `format()` allows us to format an R object for pretty printing. 
...
Keep in mind that \code{justify} does not apply to numeric values. 
...
By default, `format()` pads the strings with spaces so that they are all the 
same length.
